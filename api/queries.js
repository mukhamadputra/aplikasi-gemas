...
const getEquipmentById = (request, response) => {
  ...
  pool.query('SELECT * FROM equipments WHERE id = $1', [id], (error, results) => {
    if (error) {
      throw error
    }
    response.status(200).json(results.rows)
...
const createEquipment = (request, response) => {
  ...
  pool.query('INSERT INTO equipments (category, loc, network, status, mac_sn, th) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *', [category, loc, network, status, mac_sn, th], (error, results) => {
    if (error) {
      throw error
    }
    response.status(201).send(`Equipment added with ID: ${results.rows[0].id}`)
  })
}
...
const getReportCount = (request, response) => {
  pool.query('SELECT equipments.id, equipments.category, equipments.loc, count(reports) as total FROM equipments FULL OUTER JOIN reports ON equipments.id=reports.equip_id GROUP BY equipments.id ORDER BY total DESC', (error, results) => {
    if (error) {
      throw error
    }
    response.status(200).json(results.rows)
...
const getAllJoin = (request, response) => {
  pool.query('SELECT equipments.category as equip_cat, equipments.loc, reports.category as rep_cat, reports.time, reports.team, reports.activity FROM equipments INNER JOIN reports ON equipments.id=reports.equip_id', (error, results) => {
    if (error) {
      throw error
    }
    response.status(200).json(results.rows)
